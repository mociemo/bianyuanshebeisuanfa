...
describe('Authentication 合约', function () {
    let auth, owner, deviceWallet;

    beforeEach(async function () {
        [owner] = await ethers.getSigners();
        deviceWallet = ethers.Wallet.createRandom().connect(ethers.provider);
        await owner.sendTransaction({ to: deviceWallet.address, value: ethers.parseEther('1') });
        const Factory = await ethers.getContractFactory('Authentication');
        auth = await Factory.deploy();
        await auth.waitForDeployment();
        await auth.registerDevice(deviceWallet.address, deviceWallet.address, 1);
    });
    ...
    it('在线认证应递增 nonce 并更新 lastAuthAt', async function () {
        ...
        await auth.connect(deviceWallet).authenticate(deviceWallet.address, nonce, timestamp, '', signature);
        const info = await auth.getDeviceInfo(deviceWallet.address);
    ...
    it('错误 nonce 应导致认证失败', async function () {
        ...
        await expect(auth.connect(deviceWallet).authenticate(deviceWallet.address, 99, timestamp, '', signature)).to.be.revertedWith('Invalid nonce');
    });
    ...
    it('verifyAuth view 应正确验签', async function () {
        ...
        const timestamp = Math.floor(Date.now() / 1000);
        const signature = await signAuth(deviceWallet, nonce, timestamp, '');
        expect(await auth.verifyAuth(deviceWallet.address, nonce, timestamp, '', signature)).to.be.true;
    });
